...
CARD_DATECOMPARE={
    ...
    "body": [
        ...
        {
            "type": "TextBlock",
            "text": "The **Date Comparison** feature allows you to enter different parameters of your liking, as well as a start and end date. You will then see a comparison between the selected parameter on those dates in both values and satellite images."
        },
        {
            "type": "TextBlock",
...
compare_response = {
    ...
    "body": [    	
        ...
        {
            "type": "TextBlock",
            "text": "Japan is a island country in East Asia located in the Pacific Ocean. It comprises an archipelago of five main islands, Hokkaido, Honshu, Shikoku, Kyushu, and Okinawa. It's largest city, Tokyo, is also the country's capital.",
            "wrap": True
        }, 
        {
            "type": "TextBlock",
            "text": "Japan is widely considered to be a first world nation renowned for its technology and rich culture. It ranks very highly in both GDP and the Human Development Index, boasting the second highest life expectancy. Some major issues the country faces include its aging population and disaster-prone geography.",
            "wrap": True
        }, 
        {
            "type": "TextBlock",
        ...
        {
            "type": "TextBlock",
            "text": "The values of your selected parameter on both your start and end date are available from the _Parameter Comparison_ button below. Through the _Satellite Comparison_ button, you can also view their corresponding satellite images to visualise the differences for yourself.",
            "wrap": True
        },
        ],
        "actions":[
    ...
    api.messages.create(
           ...
           markdown=f"**Hi there! I'm a COVID-19 information bot. Here are my available functions:**\n"    	                 
                    f"\n`@Bot/help:` Details of how to use the bot\n"
                            ...
                            f"\n`@Bot/insights:` Activates the pandemic insights feature. The bot will send a card to collect your interested parameters.\n"
                            f"\n`@Bot/forecast:` Activates the parameter forecast feature. The bot will send a card to collect your interested date and parameter.\n"
                            f"\n`@Bot/detection:` Activates the object detection feature. The bot will send a card to collect your interested location and parameter.\n"
                            f"\n`@Bot/aerial-detection:` Activates the aerial detection feature. The bot will send a card to collect your interested location and parameter.\n"
...
#get end date pollution
            ...
            if float(start_pollution)>float(end_pollution):
                ...
                return "The pollution level on both "+startdate+" and "+enddate+" is **"+start_pollution+" µmol/m2**.Take note that pollution values are normally recorded once every 9 days."
            message="The pollution level on "+ startdate +" is **" + start_pollution +" µmol/m2** compared to **"+end_pollution+ " µmol/m2** on "+ enddate +\
            ". This represents **"+ str(change) + "** of **"+ str(percent_change)+"%**"
            return message 
    ...
    try:
        ...
        if datetime.datetime.strptime(start,'%Y-%m-%d').date()>=datetime.datetime.strptime(end,'%Y-%m-%d').date():
            ...
            if float(start_veg)>float(end_veg):
                ...
                return "The vegetation index on both "+ str(start_date)+" and "+ str(end_date) + " is **"+ str(round(start_veg,3))+ "**. Take note that vegetation index values are normally recorded once every 16 days."
            if change=="a decrease" or "an increase":
                message="The vegetation index on "+ str(start_date) +" is **" + str(round(start_veg,2)) +"** compared to **"+str((round(end_veg,2)))+ "** on "+ str(end_date) +". This represents **"+ str(change) + "** of **"+ str(percent_change)+"%**."  
                return message 
    except:
        return "Please ensure that the date inputs are not in the future. Otherwise, information on the requested date is not available"
...
def respond_to_button_press(webhook):
    ...
    if attachment_action.json_data["inputs"]["formDemoAction"]=="SubmitA":
        api.messages.create(
                room.id,    
                markdown=f"Request submitted for *Date Comparison Feature*"
            )
        ...
        if start_date or end_date=='':
            ...
            if type(image_output(start_date, end_date))!=str:
                api.messages.create(
                room.id,
                markdown=f"Retrieving information from our database..."
            )
            ...
            },
            {
                "key": "text",
                "target": "__TEXT2__",
                "val": f"For reference, the mean pollution levels in Japan during the pandemic (24 January, 2020 to present) is **27.48 µmol/m2** while the pollution levels before the pandemic is **32.19 µmol/m2**.",
            },
            {
                "key": "text",
                "target": "__TEXT3__",
                "val": f"You can visualise the difference from the _Satellite Comparison_ button provided.",
            },
            {
                "key": "text",
                "target": "__TEXT4__",
                "val": f"The above image shows a side-by-side comparison of the mean NO2 emissions in the tropposhere (0-10 km above ground)."            
           },
           {
                "key": "text",
                "target": "__TEXT5__",
                "val": f"The left image corresponds to the pollution levels recorded on your start date while the right corresponds to that of your end date."            
           },
           {
                "key": "text",
                "target": "__TEXT6__",
                "val": f"Data was collected from _Sentinel 5P Tropomi Satellite_ with layout provided by _Tropomi Explorer_ application (created by Justin Braaten)"
           },
           ])
        ...
        elif attachment_action.json_data["inputs"]["CompareChoiceVal"]=="Vegetation Index":
            ...
            },
            {
                "key": "text",
                "target": "__TEXT2__",
                "val": f"For reference, the mean vegetation index before and after the pandemic is **5882.287** and **5648.326** respectively. ",
            },
            {
                "key": "text",
                "target": "__TEXT3__",
            ...
            },
            {
                "key": "text",
                "target": "__TEXT4__",
                "val": f"The above image shows a side-by-side comparison of the mean Vegetation Index recorded using NDVI in your chosen country."
            },
            {
                "key": "text",
                "target": "__TEXT5__",
                "val": f" The left image corresponds to the NDVI recorded on your start date while the right corresponds to that of your end date."            
           },
            {
                "key": "text",
                "target": "__TEXT6__",
                "val": f"Data was collected from _MODIS Terra Earth Observation Satellite_ with layout provided using Google Earth Engine Code Editor.",
            },
            ])
                ret = api.messages.create(
            roomId=room.id,
    ...
    if attachment_action.json_data["inputs"]["formDemoAction"]=="SubmitB":
        api.messages.create(
                ...
                markdown=f"Request submitted for *Pandemic Insights Feature*"
            )
        api.messages.create(
                room.id,
        ...
        if corr_image()!="null":
            ...
            api.messages.create(
                room.id,
                markdown=f"No parameters were selected. No results will hence be available."
            )  
        
    if attachment_action.json_data["inputs"]["formDemoAction"]=="SubmitC":
        api.messages.create(
                room.id,
                markdown=f"Request submitted for *Parameter Forecast Feature*"
            )
        forecast_date=attachment_action.json_data["inputs"]["forecast_date"]
        if forecast_date=='':
            api.messages.create(
                room.id,
                markdown=f"No dates were selected for forecasting. No results will hence be available."
            )      
        elif datetime.datetime.strptime(forecast_date, '%Y-%m-%d')<datetime.datetime.now():
            api.messages.create(
                room.id,
...
def respond_to_message(webhook):
    ...
    if message.personId == me.id:
        ...
        if "/features" in message.text:
            api.messages.create(
                   room.id,
                   markdown=f"This bot is equipped with 5 functions. Below is the list of available functions and what they each do.\n"
                   ...
                   f"\n**Object Detection:** This feature allows you to enter a location of your liking, as well as a parameter. Using low-level surveillance cameras, you will then be able to see a cluster analysis of your chosen parameter in your selected location, as well as its relation to social distancing and COVID-19.\n"
                   f"\n**Aerial Detection:** This feature allows you to enter a location of your liking, as well as a parameter. Using high-altitude satellite images, you will then be able to see a cluster analysis of your chosen parameter in your selected location, as well as its relation to social distancing and COVID-19."
                   )
